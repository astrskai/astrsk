        ...
        if (context.operation === 'set') {
          ...
          if (flowId && context.value) {
            ...
            if (result.isSuccess) {
              ...
              throw new Error(`Failed to create/update data store node: ${result.getError()}`);
            }
          } else {
          ...
          if (flowId) {
            ...
            const result = await DataStoreNodeService.deleteDataStoreNode.execute({
              nodeId: nodeId
            });
            ...
            if (result.isSuccess) {
              ...
              throw new Error(`Failed to delete data store node: ${result.getError()}`);
            }
          } else {
            // Fallback to local update if no flowId available
            delete resource.dataStoreNodes[nodeId];
  ...
  field: {
    ...
    handler: async (context: OperationContext, match: PathMatchResult) => {
      try {
        ...
        if (!resource.dataStoreNodes) resource.dataStoreNodes = {};
        if (!resource.dataStoreNodes[nodeId]) resource.dataStoreNodes[nodeId] = {};
        ...
        if (fieldName === 'dataStoreFields' && context.operation === 'put') {
          ...
          const currentFieldsArray = resource.dataStoreNodes[nodeId].dataStoreFields;
          const existingFields = Array.isArray(currentFieldsArray) ? [...currentFieldsArray] : [];
          ...
          if (context.value && typeof context.value.index === 'number') {
            ...
            const newField = {
              id: context.value.id || new (await import('@/shared/domain')).UniqueEntityID().toString(),
          ...
          if (flowId) {
            ...
            if (result.isSuccess) {
              ...
              try {
                ...
                console.warn('⚠️ [DATA-STORE-PROCESSOR] Could not invalidate data store node queries:', invalidationError);
              }
            } else {
              throw new Error(`Failed to update data store fields: ${result.getError()}`);
            }
          } else {
            // Fallback to local update if no flowId available
            resource.dataStoreNodes[nodeId].dataStoreFields = existingFields;
            ...
            if (flowId) {
              ...
              if (fieldName === 'name') {
                ...
                if (result.isSuccess) {
                  ...
                  throw new Error(`Failed to update data store name: ${result.getError()}`);
                }
                ...
                if (result.isSuccess) {
                  ...
                  throw new Error(`Failed to update data store color: ${result.getError()}`);
                }
              } else {
  ...
  dataStoreFieldsIndexed: {
    ...
    handler: (context: OperationContext, match: PathMatchResult) => {
      try {
        ...
        while (resource.dataStoreNodes[nodeId].dataStoreFields.length <= index) {
          resource.dataStoreNodes[nodeId].dataStoreFields.push({});
        }
        
        if (context.operation === 'set') {
          resource.dataStoreNodes[nodeId].dataStoreFields[index] = context.value;
        } else if (context.operation === 'put') {
          // PUT should INSERT at the index, shifting existing items
          resource.dataStoreNodes[nodeId].dataStoreFields.splice(index, 0, context.value);
        } else if (context.operation === 'remove') {
