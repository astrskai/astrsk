...
export class CreateDataStoreNodeUseCase
  implements UseCase<CreateDataStoreNodeRequest, Result<DataStoreNode>>
{
  constructor(private saveDataStoreNodeRepo: SaveDataStoreNodeRepo) {}

  async execute(
    request: CreateDataStoreNodeRequest,
  ): Promise<Result<DataStoreNode>> {
    try {
      ...
      // Save to repository
      const saveResult =
        await this.saveDataStoreNodeRepo.saveDataStoreNode(dataStoreNode);
      ...
      return Result.fail(
        `Failed to create data store node: ${(error as any)?.message || String(error)}`,
      );
    }
  }
}
