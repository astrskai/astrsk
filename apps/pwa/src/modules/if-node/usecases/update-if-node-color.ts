...
export interface UpdateIfNodeColorRequest {
  flowId: string;
  nodeId: string;
  color: string;
}

export class UpdateIfNodeColorUseCase {
  constructor(
    private loadIfNodeRepo: LoadIfNodeRepo,
    private saveIfNodeRepo: SaveIfNodeRepo,
  ) {}

  async execute(request: UpdateIfNodeColorRequest): Promise<Result<IfNode>> {
    try {
      ...
      // Update the color
      const updateResult = ifNode.updateColor(request.color);
      ...
      console.error("Failed to update if node color:", error);
      return Result.fail(`Failed to update if node color: ${error}`);
    }
  }
}