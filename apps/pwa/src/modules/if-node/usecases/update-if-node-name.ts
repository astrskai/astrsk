...
export interface UpdateIfNodeNameRequest {
  flowId: string;
  nodeId: string;
  name: string;
}

export class UpdateIfNodeNameUseCase {
  constructor(
    private loadIfNodeRepo: LoadIfNodeRepo,
    private saveIfNodeRepo: SaveIfNodeRepo,
  ) {}

  async execute(request: UpdateIfNodeNameRequest): Promise<Result<IfNode>> {
    try {
      ...
      // Update the name
      const updateResult = ifNode.updateName(request.name);
      ...
      console.error("Failed to update if node name:", error);
      return Result.fail(`Failed to update if node name: ${error}`);
    }
  }
}