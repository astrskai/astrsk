  ...
  return useMutation({
    ...
    onMutate: async ({ nodeType, position, data }) => {
      ...
      const predeterminedId =
        data.id || data.nodeId || `temp-node-${Date.now()}`;
  ...
  return useMutation({
    ...
    onMutate: async ({ nodeId }) => {
      ...
      queryClient.setQueryData(flowKeys.detail(flowId), (old: any) => {
        ...
        const updatedNodes = old.nodes.filter((n: any) => n.id !== nodeId);
        const updatedEdges = (old.edges || []).filter(
  ...
  return useMutation({
    ...
    onMutate: async ({ source, target, label, id }) => {
      ...
      queryClient.setQueryData(flowKeys.detail(flowId), (old: any) => {
        ...
        const existingEdge = (old.edges || []).find(
        ...
        const updatedEdges = [...(old.edges || []), newEdge];
...
export const useRemoveEdge = (flowId: string) => {
  ...
  return useMutation({
    ...
    mutationFn: async ({ edgeId }: RemoveEdgeRequest) => {
      ...
      const edgeIndex = (flow.props.edges || []).findIndex(
        (e: any) => e.id === edgeId,
      );
      if (edgeIndex === -1) {
