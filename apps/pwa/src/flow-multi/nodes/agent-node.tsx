 ...
 */
function AgentNodeComponent({ agentId, flow, nodeId, selected }: AgentNodeComponentProps) {
  ...
  const colorWithOpacity = useMemo(() => {
    ...
    return agentOpacity < 1 
      ? `${agentColor}${Math.round(agentOpacity * 255).toString(16).padStart(2, '0')}`
      : agentColor;
  ...
  const { promptType, parameterCount, hasPrompt, hasAgentName, hasStructuredOutput, isConnectedStartToEnd } = useMemo(() => {
    ...
    if (flow && flow.props?.nodes && flow.props?.edges) {
      try {
        ...
        isConnected = !!(agentPosition && agentPosition.isConnectedToStart && agentPosition.isConnectedToEnd);
      } catch (error) {
    ...
    return {
      ...
      parameterCount: paramData?.enabledParameters ? 
        Array.from(paramData.enabledParameters.values()).filter(enabled => enabled === true).length : 0,
  ...
  const saveAgentName = useCallback(async (newName: string, currentName: string) => {
    ...
    try {
      ...
      const { updatedAgents, responseTemplateChanged, totalReferencesUpdated, updatedResponseTemplate, updatedNodes } = 
        await updateAgentNameReferences(currentName, trimmedName, agentId, currentFlow);
      ...
      await queryClient.invalidateQueries({ queryKey: agentKeys.name(agentId) });
      ...
      for (const agentWithUpdatedRefs of updatedAgents) {
        ...
        if (agentWithUpdatedRefs.props.promptMessages || agentWithUpdatedRefs.props.textPrompt !== undefined) {
          const result = await AgentService.updateAgentPrompt.execute({
            agentId: targetAgentId,
            ...(agentWithUpdatedRefs.props.promptMessages && { promptMessages: agentWithUpdatedRefs.props.promptMessages }),
            ...(agentWithUpdatedRefs.props.textPrompt !== undefined && { textPrompt: agentWithUpdatedRefs.props.textPrompt })
          });
          if (result.isFailure) {
            console.error(`Failed to update prompt for agent ${targetAgentId}:`, result.getError());
          } else {
            // Manually invalidate queries to trigger UI updates
            await queryClient.invalidateQueries({ queryKey: agentKeys.prompt(targetAgentId) });
          }
        }
        ...
        if (agentWithUpdatedRefs.props.schemaFields) {
          ...
          if (result.isFailure) {
            console.error(`Failed to update output for agent ${targetAgentId}:`, result.getError());
          } else {
            // Manually invalidate queries to trigger UI updates
            await queryClient.invalidateQueries({ queryKey: agentKeys.output(targetAgentId) });
          }
        }
      }
        ...
        for (const node of updatedNodes) {
          if (node.type === 'if' && node.data?.conditions) {
            ...
            const result = await FlowService.updateIfNodeConditions.execute({
              ...
              draftConditions: node.data.draftConditions || node.data.conditions,
              logicOperator: node.data.logicOperator
            });
            if (result.isFailure) {
              console.error(`Failed to update if-node ${node.id}:`, result.getError());
            } else {
              // Invalidate if-node queries
              await queryClient.invalidateQueries({ queryKey: flowKeys.node(flowId, node.id) });
            }
          } else if (node.type === 'dataStore' && node.data?.dataStoreFields) {
            ...
            if (result.isFailure) {
              console.error(`Failed to update data-store node ${node.id}:`, result.getError());
            } else {
              // Invalidate data-store node queries  
              await queryClient.invalidateQueries({ queryKey: flowKeys.dataStoreRuntime(flowId, node.id) });
            }
          }
        }
      }
  ...
  }, [agentId, flowId, updateNameMutation, notifyAgentUpdate, updateAgentPanelStates, updateAgentNameReferences, queryClient]);
  ...
  const handleCopyClick = useCallback(async (e: React.MouseEvent) => {
    ...
    if ((window as any).flowPanelCopyAgent) {
      (window as any).flowPanelCopyAgent(agentId);
    } else {
  ...
  const handleDelete = useCallback(async () => {
    ...
    if ((window as any).flowPanelDeleteAgent) {
      (window as any).flowPanelDeleteAgent(agentId);
    } else {
      ...
      <Dialog open={isOpenDelete} onOpenChange={setIsOpenDelete}>
        <DialogContent hideClose>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete agent "{nameData?.name || 'Unnamed'}"? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
...
export default function AgentNode({
  ...
  const { data: nameData, isLoading, error } = useQuery({
    ...agentQueries.name(agentId),
    retry: false,
  });
  ...
  return <AgentNodeComponent agentId={agentId} flow={selectedFlow} nodeId={id} selected={selected} />;
}