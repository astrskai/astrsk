...
export const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  const hasAppliedChanges = message.sessionId && appliedChanges.some(
    change => change.sessionId === message.sessionId
  );
  ...
  if (message.type === 'edit_approval') {
    return (
      <div className="flex flex-col self-stretch">
        ...
        <div className="flex flex-col gap-4" data-state={message.status === 'pending' ? 'Default' : message.status === 'approved' ? 'Approved' : 'Revert'} data-type="Vibe coder">
          <EditApprovalMessage
            message={message}
            onApprove={onApprove!}
            onReject={onReject!}
  ...
  if (message.type === 'analysis_ready') {
    return (
      <div className="flex flex-col gap-4 self-stretch">
        ...
        <div className="flex flex-col gap-4" data-state="Default" data-type="Vibe coder">
  ...
  return (
    <>
      {message.role === 'user' ? (
        <div data-state="Default" data-type="User" className="self-stretch flex flex-col justify-start items-end gap-10">
        ...
        <div data-state="Default" data-type="Vibe coder" className="self-stretch flex flex-col justify-start items-start gap-4">
          ...
          {message.analysis?.isSimpleAnswer && message.analysis.simpleAnswer && (
