...
import { useUpdateAgentPromptMessages, useUpdateAgentApiType, useUpdateAgentTextPrompt } from '@/app/queries/agent/mutations/prompt-mutations-new';
import { useUpdateAgentOutput, useUpdateAgentOutputFormat } from '@/app/queries/agent/mutations/output-mutations';
...
import type { EditableFlowData, EditableAgentData, EditableIfNodeData, EditableDataStoreNodeData } from 'vibe-shared-types';
import { OutputFormat } from '@/modules/agent/domain/agent';

// Helper function to invalidate flow queries
const invalidateSingleFlowQueries = async (queryClient: any, flowId: string) => {
...
export const useApplyFlowChanges = ({ 
   ...
   */
  const applyAgentChanges = useCallback(async (nodeId: string, agentData: any) => {
    ...
    try {
      ...
      const snakeCaseData = {
        ...
        prompt_messages: agentData.promptMessages ? stringify(agentData.promptMessages) : '[]',
        ...
        output_streaming: agentData.outputStreaming !== undefined ? agentData.outputStreaming : true,
      ...
      console.log('[APPLY-AGENT-CHANGES] Converted to domain object:', {
        ...
        hasPromptMessages: agent.props.promptMessages ? agent.props.promptMessages.length > 0 : false,
      ...
      if (result.isSuccess) {
        console.log('[APPLY-AGENT-CHANGES] Successfully saved complete agent');
        // Invalidate queries to refresh UI
        await queryClient.invalidateQueries({ queryKey: agentKeys.prompt(nodeId) });
        await queryClient.invalidateQueries({ queryKey: agentKeys.detail(nodeId) });
        await queryClient.invalidateQueries({ queryKey: agentKeys.output(nodeId) });
      } else {
   ...
   */
  const applyIfNodeChanges = useCallback(async (nodeId: string, ifNodeData: EditableIfNodeData) => {
    const changes: string[] = [];
    
    // Update conditions and logic operator using FlowService (which supports draftConditions)
    if (ifNodeData.conditions !== undefined || ifNodeData.logicOperator !== undefined) {
      ...
      if (result.isSuccess) {
        ...
        await queryClient.invalidateQueries({ queryKey: ifNodeKeys.conditions(flowId, nodeId) });
    ...
    if (ifNodeData.name !== undefined) {
      ...
      if (result.isSuccess) {
        changes.push('name');
        await queryClient.invalidateQueries({ queryKey: ifNodeKeys.name(flowId, nodeId) });
        await queryClient.invalidateQueries({ queryKey: ifNodeKeys.detail(flowId, nodeId) });
      } else {
        throw new Error(`Failed to update if-node name: ${result.getError()}`);
      }
    }
   ...
   */
  const applyDataStoreNodeChanges = useCallback(async (nodeId: string, dataStoreData: EditableDataStoreNodeData) => {
    const changes: string[] = [];
    ...
    if (dataStoreData.dataStoreFields !== undefined) {
      ...
      if (result.isSuccess) {
        ...
        await queryClient.invalidateQueries({ queryKey: dataStoreNodeKeys.fields(flowId, nodeId) });
    ...
    if (dataStoreData.name !== undefined) {
      ...
      if (result.isSuccess) {
        changes.push('name');
        await queryClient.invalidateQueries({ queryKey: dataStoreNodeKeys.name(flowId, nodeId) });
        await queryClient.invalidateQueries({ queryKey: dataStoreNodeKeys.detail(flowId, nodeId) });
      }
    }
   ...
   */
  const applyFlowChanges = useCallback(async (editedData: EditableFlowData) => {
    ...
    try {
      ...
      if (editedData.response_template !== undefined && editedData.response_template !== currentFlow?.props?.responseTemplate) {
        try {
      ...
      if (editedData.data_store_schema !== undefined) {
        const hasDataStoreChanges = JSON.stringify(currentFlow?.props?.dataStoreSchema) !== JSON.stringify(editedData.data_store_schema);
        if (hasDataStoreChanges) {
          try {
            await updateDataStoreSchema.mutateAsync(editedData.data_store_schema);
      ...
      const hasNodeChanges = editedData.nodes && !areNodesEqual(currentFlow?.props?.nodes, editedData.nodes);
      const hasEdgeChanges = editedData.edges && !areEdgesEqual(currentFlow?.props?.edges, editedData.edges);
      
      if (hasNodeChanges || hasEdgeChanges) {
        // Update flow structure (nodes and edges)
        try {
          await updateNodesAndEdges.mutateAsync({
            nodes: hasNodeChanges ? editedData.nodes as any : currentFlow?.props?.nodes || [],
            edges: hasEdgeChanges ? editedData.edges as any : currentFlow?.props?.edges || []
          });
      ...
      if (editedData.ifNodes) {
        for (const [nodeId, ifNodeData] of Object.entries(editedData.ifNodes)) {
          try {
            await applyIfNodeChanges(nodeId, ifNodeData as EditableIfNodeData);
      ...
      if (editedData.dataStoreNodes) {
        for (const [nodeId, dataStoreData] of Object.entries(editedData.dataStoreNodes)) {
          try {
            await applyDataStoreNodeChanges(nodeId, dataStoreData as EditableDataStoreNodeData);
   ...
   */
  const applyChanges = useCallback(async (editedData: EditableFlowData) => {
    ...
    if (result.errors.length === 0) {
      toast.success(`Successfully applied ${result.appliedFields.length} flow changes`);
      onSuccess?.();
    } else {
