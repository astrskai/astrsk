 ...
 *
 * @remarks
 * Similar to useUpdateCharacterCard - non-atomic batch update.
 * Individual field updates are executed in parallel.
 */
export const useUpdatePlotCard = (cardId: string) => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: {
      ...
      if (data.description !== undefined) {
        promises.push(
          CardService.updatePlotDescription.execute({
            cardId,
            description: data.description,
          }),
        );
      ...
      if (data.scenarios !== undefined) {
        promises.push(
          CardService.updateCardScenarios.execute({
            cardId,
