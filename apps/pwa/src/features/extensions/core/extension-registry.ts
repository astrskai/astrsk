...
export class ExtensionRegistry {
  ...
  /**
   * Get all UI components for a specific slot
   * Components are sorted by order (lower = first)
   */
  getUIComponentsForSlot(slot: string): ExtensionUIComponent[] {
    const components: ExtensionUIComponent[] = [];

    // Collect components from all extensions
    for (const client of this.clients.values()) {
      const clientComponents = client.getUIComponents();
      for (const component of clientComponents.values()) {
    ...
    // Sort by order (lower = first), default to 0
