...
export const useApplyCardChanges = ({
   ...
   */
  const applyCharacterCardChanges = useCallback(
    async (editedData: EditableCharacterCard) => {
      ...
      try {
        ...
        if (
          editedData.common?.title &&
          editedData.common.title !== currentCard?.props?.title
        ) {
        ...
        if (editedData.character?.name) {
          ...
          if (editedData.character.name !== currentName) {
            ...
            if (result.isSuccess) {
              ...
              errors.push(
                `Failed to update character name: ${result.getError()}`,
              );
            }
          }
        }
        ...
        if (editedData.character?.lorebook) {
          ...
          if (hasLorebookChanges) {
            ...
            const lorebookEntries = editedData.character.lorebook.entries.map(
              (entry) =>
                Entry.create({
   ...
   */
  const applyPlotCardChanges = useCallback(
    async (editedData: EditablePlotCard) => {
      const appliedFields: string[] = [];
      const errors: string[] = [];
      ...
      try {
        ...
        if (editedData.plot?.scenarios) {
          ...
          const hasScenariosChanges = !areScenariosEqual(
            currentScenarios,
            editedData.plot.scenarios,
          );
        ...
        if (editedData.plot?.lorebook) {
          ...
          if (hasLorebookChanges) {
            const lorebookEntries = editedData.plot.lorebook.entries.map(
              (entry) =>
                Entry.create({
                  id: new UniqueEntityID(entry.id),
                  name: entry.name,
  ...
  const applyChanges = useCallback(
    async (editedData: EditableCharacterCard | EditablePlotCard) => {
      let result;

      // Determine card type from edited data structure
      if ("character" in editedData && editedData.character) {
        result = await applyCharacterCardChanges(
          editedData as EditableCharacterCard,
        );
    ...
    [
      applyCharacterCardChanges,
      applyPlotCardChanges,
      cardId,
      onSuccess,
      onError,
    ],
 ...
 */
function areLorebooksEqual(
  current: Lorebook | undefined,
  edited: { entries: LorebookEntry[] },
): boolean {
...
export const extractCardFields = (
  vibeData: any,
): EditableCharacterCard | EditablePlotCard => {
