import { ValidationIssueCode } from "@/features/flow/flow-multi/validation/types/validation-types";
import {
  ValidationData,
  BaseValidationData,
  ...
  isSyntaxErrorData,
} from "@/features/flow/flow-multi/validation/types/message-data-types";
...
const agentName = (data?: { agentName?: string }) =>
  data?.agentName || "Unnamed";

// Message templates for common patterns
const templates = {
  missingInAgent: (item: string) => ({
    title: `Missing ${item}`,
    description: (data?: BaseValidationData) =>
      `Agent "${agentName(data)}" has no ${item}`,
  ...
  invalidStructure: (item: string, requirement: string) => ({
    ...
    description: (data?: BaseValidationData) =>
      `Agent "${agentName(data)}" ${requirement}`,
    suggestion: (fix: string) => fix,
  }),

  unsupported: (feature: string) => ({
    title: (data?: ProviderValidationData) =>
      `Unsupported ${data?.feature || feature}`,
    ...
    suggestion: (data?: ProviderValidationData) =>
      `Disable ${data?.feature || feature} or use a model that supports it`,
  }),
};

// Compact message definitions
const messageDefinitions: Record<
  ValidationIssueCode,
  {
...
> = {
  ...
  [ValidationIssueCode.NO_MODEL_SELECTED]: {
    ...
    description: (data?: ValidationData) =>
      `Agent "${agentName(data)}" does not have a model`,
  ...
  [ValidationIssueCode.MODEL_NOT_AVAILABLE]: {
    ...
    description: (data?: ValidationData) =>
      `Agent "${agentName(data)}" uses model that is not available`,
    suggestion:
      "Select a different model or connect to the selected provider to use this model",
  },
  ...
  [ValidationIssueCode.AGENT_NAME_TOO_SHORT]: {
    ...
    description: (data?: ValidationData) =>
      `Agent "${agentName(data)}" name must be at least 3 characters long`,
  ...
  [ValidationIssueCode.DUPLICATE_AGENT_NAME]: {
    ...
    description: (data?: ValidationData) =>
      `Two or more  agents have the name "${agentName(data)}"`,
  ...
  [ValidationIssueCode.MISSING_STRUCTURED_OUTPUT_SCHEMA]: {
    ...
    description: (data?: ValidationData) =>
      `Agent "${agentName(data)}" has no schema fields in its structured output`,
  ...
  [ValidationIssueCode.SYSTEM_MESSAGE_IN_MIDDLE]: {
    ...
    description: (data?: ValidationData) =>
      `Agent "${agentName(data)}" has a system message that appears after user or assistant messages`,
  ...
  [ValidationIssueCode.IF_NODE_MISSING_BRANCHES]: {
    ...
    suggestion:
      "Connect both true and false branches of the if-node to downstream nodes",
  },
  ...
  [ValidationIssueCode.IF_NODE_BRANCH_NOT_REACHING_END]: {
    ...
    suggestion:
      "Ensure all branches of if-nodes eventually connect to the end node",
  },
  ...
  [ValidationIssueCode.MISSING_HISTORY_MESSAGE]: {
    ...
    description: (data?: ValidationData) =>
      `Agent "${agentName(data)}"'s prompt does not contain a history message`,
    suggestion:
      "Add a history message or {{history}} variable to include the session history",
  },

  [ValidationIssueCode.UNSUPPORTED_PARAMETERS]:
    templates.unsupported("feature"),
  ...
  [ValidationIssueCode.UNDEFINED_PROVIDER_PARAMETER]: {
    ...
    suggestion:
      "This parameter is not supported by the model provider. Check provider documentation or disable this parameter",
  },
  ...
  [ValidationIssueCode.DATA_STORE_INVALID_INITIAL_VALUE]: {
    ...
    description: (data?: ValidationData) =>
      `Data update "${agentName(data)}" has an invalid initial value that doesn't match the schema`,
  ...
  [ValidationIssueCode.DATA_STORE_MISSING_INITIAL_VALUE]: {
    ...
    description: (data?: ValidationData) =>
      `Data update "${agentName(data)}" is missing an initial value`,
...
export function generateValidationMessage(
  ...
  return {
    ...
    suggestion:
      typeof definition.suggestion === "function"
        ? definition.suggestion(data)
