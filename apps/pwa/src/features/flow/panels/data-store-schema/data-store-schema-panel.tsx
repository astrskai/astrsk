...
export function DataStoreSchemaPanel({ flowId }: DataStoreSchemaProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-surface-raised">
      ...
      {fields.length === 0 ? (
        // Empty state
        <div className="self-stretch flex-1 p-2 bg-surface-raised inline-flex justify-start items-start gap-2">
          <div className="flex-1 self-stretch min-w-36 inline-flex flex-col justify-start items-start gap-4">
            <div className="self-stretch flex-1 flex flex-col justify-center items-center gap-8">
              ...
              <button 
                onClick={addNewField}
                className="h-7 px-3 py-2 bg-hover rounded-full shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-border-subtle inline-flex justify-center items-center gap-2 hover:bg-active transition-colors"
              >
                <Plus className="w-4 h-4 text-fg-muted" />
                <div className="justify-center text-fg-default text-xs font-semibold leading-none">Create schema field</div>
              </button>
            </div>
          </div>
        </div>
      ) : (
        <div className="flex-1 p-2 bg-surface-raised inline-flex justify-start items-start gap-2">
          {/* Left panel - Schema fields */}
          <div className="flex-1 max-w-64 min-w-36 inline-flex flex-col justify-start items-start gap-4">
            <button
              onClick={addNewField}
              className="h-7 px-3 py-2 bg-hover rounded-full shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline-1 outline-offset-[-1px] outline-border-subtle inline-flex justify-center items-center gap-2 hover:bg-active transition-colors"
            >
              <Plus className="w-4 h-4 text-fg-muted" />
          ...
          {selectedField ? (
            <>
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <Trash2 className="size-[24px] text-fg-subtle" />
                    </button>
                  </TooltipTrigger>
              ...
              {selectedField.type === 'boolean' ? (
                <div className="self-stretch flex flex-col justify-start items-start gap-1">
                  <div className="self-stretch inline-flex justify-start items-start gap-2">
                    <div className="justify-start text-fg-muted text-[10px] font-medium leading-none">Initial state</div>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                  ...
                  <div className="self-stretch flex flex-col justify-start items-start gap-1">
                    ...
                    >
                      ...
                      <div className="justify-start text-fg-default text-[10px] font-medium leading-none">True</div>
                    </button>
                    <button
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="justify-start text-fg-default text-[10px] font-medium leading-none">False</div>
                    </button>
                  </div>
                </div>
              ) : (
                <div className="self-stretch inline-flex flex-col justify-start items-start gap-2">
                  <div className="self-stretch inline-flex justify-start items-center gap-2">
                    <div className="justify-start text-fg-muted text-[10px] font-medium leading-none">Initial value</div>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <HelpCircle className="w-4 h-4 text-fg-subtle" />
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <div className="text-fg-subtle text-xs mb-2">No field selected</div>
